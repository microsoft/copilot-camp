...
Select the **AI actions** command 1️⃣, then select the **Create an agent** 2️⃣ command in the command bar.
...
Select the **...** of the **HR agent.agent** file in the document library, select to open the ECB menu of SharePoint Online and select the **Edit** command. As a suitable alternative, you can also select the **Edit** command in the library command bar, if you like.
...
A new dialog window shows up and allows you to manage the following settings:
...
There is a section to **Add a source** 1️⃣, which allows you to add additional site collections to the data sources of the agent or specific files and folders from OneDrive for Business. You can search sites by title, or you can simply provide the actual URL of the site that you want to add. You can also select **Add contents from this site** 2️⃣, as long as the overall number of data source items will not exceed the limit of 20 items.
Lastly, you can select the **Remove this site and all contents** 3️⃣ command to remove an already selected site. Be careful that, in case you remove the current site, from which you've built the agent, the configuration tool will completely remove all the documents that you selected when you initially configured the agent.
...
Once you have updated the agent icon, purpose, and instructions select the agent and interact with it again.
...
You can notice that the agent will reply to the "Hello!" message with a comprehensive welcome message. Moreover, responses are more accurate, with emojis here and there, and with tables to render lists of items, accordingly to the provided instructions.
